...
def telemetry(sid, data):
    ...
    X = crop_images(image_array, (0, 45, 0, 20))
    X = cv2.resize(X.reshape(X.shape[1:]), (200, 66), cv2.INTER_AREA)
    # X = transform.resize(X.reshape(X.shape[1:]), (66, 200))*255
    # X = to_YUV(X.astype(np.uint8))
